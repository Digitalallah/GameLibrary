
const ORIGINAL_GAMES = [
  {
    "id": "220",
    "title": "Half-Life 2",
    "releaseDate": "2004-11-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/220/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/220/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "220",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "280",
    "title": "Half-Life: Source",
    "releaseDate": "2004-06-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/280/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/280/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "280",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "320",
    "title": "Half-Life 2: Deathmatch",
    "releaseDate": "2004-11-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/320/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/320/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "320",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "360",
    "title": "Half-Life Deathmatch: Source",
    "releaseDate": "2006-05-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "400",
    "title": "Portal",
    "releaseDate": "2007-10-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/400/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/400/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "400",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "500",
    "title": "Left 4 Dead",
    "releaseDate": "2008-11-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "550",
    "title": "Left 4 Dead 2",
    "releaseDate": "2009-11-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/550/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/550/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "550",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "620",
    "title": "Portal 2",
    "releaseDate": "2011-04-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/620/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/620/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "620",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "730",
    "title": "Counter-Strike 2",
    "releaseDate": "2012-08-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/730/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/730/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "730",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1500",
    "title": "Darwinia",
    "releaseDate": "2005-12-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2990",
    "title": "FlatOut 2",
    "releaseDate": "2006-12-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2990/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2990/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2990",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "6220",
    "title": "FlatOut",
    "releaseDate": "2007-02-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/6220/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/6220/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "6220",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "6910",
    "title": "Deus Ex: Game of the Year Edition",
    "releaseDate": "2007-03-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/6910/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/6910/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "6910",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "6920",
    "title": "Deus Ex: Invisible War",
    "releaseDate": "2007-03-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/6920/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/6920/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "6920",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "12320",
    "title": "Sacred Gold",
    "releaseDate": "2008-07-09",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/12320/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/12320/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "12320",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "12360",
    "title": "FlatOut: Ultimate Carnage Collector's Edition",
    "releaseDate": "2008-08-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/12360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/12360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "12360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "15130",
    "title": "Beyond Good & Evil",
    "releaseDate": "2008-05-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/15130/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/15130/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "15130",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "17460",
    "title": "Mass Effect (2007)",
    "releaseDate": "2008-12-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/17460/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/17460/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "17460",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "20900",
    "title": "The Witcher: Enhanced Edition",
    "releaseDate": "2008-09-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/20900/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/20900/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "20900",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "22320",
    "title": "The Elder Scrolls III: Morrowind",
    "releaseDate": "2009-06-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/22320/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/22320/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "22320",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "22330",
    "title": "The Elder Scrolls IV: Oblivion Game of the Year Edition (2009)",
    "releaseDate": "2009-06-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/22330/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/22330/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "22330",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "22490",
    "title": "Fallout: New Vegas PCR",
    "releaseDate": "",
    "releaseDateStatus": "unknown",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/22490/",
    "addedAt": "2026-06-22",
    "steamAppId": "22490",
    "sources": [
      "https://store.steampowered.com/app/22490/"
    ],
    "releaseDatePrecision": "unknown",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "24980",
    "title": "Mass Effect 2 (2010)",
    "releaseDate": "2010-01-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/24980/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/24980/",
      "https://en.wikipedia.org/wiki/Mass_Effect_2"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "24980",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "35140",
    "title": "Batman: Arkham Asylum GOTY Edition",
    "releaseDate": "2010-03-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/35140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/35140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "35140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "38400",
    "title": "Fallout",
    "releaseDate": "2015-11-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/38400/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/38400/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "38400",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "38410",
    "title": "Fallout 2",
    "releaseDate": "2009-08-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/38410/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/38410/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "38410",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "38420",
    "title": "Fallout Tactics",
    "releaseDate": "2009-08-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/38420/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/38420/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "38420",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "39500",
    "title": "Gothic 3",
    "releaseDate": "2009-12-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/39500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/39500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "39500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "39510",
    "title": "Gothic II: Gold Classic",
    "releaseDate": "2005-11-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/39510/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/39510/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "39510",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "41000",
    "title": "Serious Sam HD: The First Encounter",
    "releaseDate": "2009-11-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/41000/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/41000/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "41000",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "41010",
    "title": "Serious Sam HD: The Second Encounter",
    "releaseDate": "",
    "releaseDateStatus": "unknown",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/41010/",
    "addedAt": "2026-06-22",
    "steamAppId": "41010",
    "sources": [
      "https://store.steampowered.com/app/41010/"
    ],
    "releaseDatePrecision": "unknown",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "43160",
    "title": "Metro: Last Light Complete Edition",
    "releaseDate": "",
    "releaseDateStatus": "unknown",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/43160/",
    "addedAt": "2026-06-22",
    "steamAppId": "43160",
    "sources": [
      "https://store.steampowered.com/app/43160/"
    ],
    "releaseDatePrecision": "unknown",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "57300",
    "title": "Amnesia: The Dark Descent",
    "releaseDate": "2010-09-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/57300/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/57300/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "57300",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "65540",
    "title": "Gothic 1 Classic",
    "releaseDate": "2001-03-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/65540/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/65540/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "65540",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "108600",
    "title": "Project Zomboid",
    "releaseDate": "2013-11-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2013-11-08",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/108600/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/108600/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "108600",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "108710",
    "title": "Alan Wake",
    "releaseDate": "2012-02-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/108710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/108710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "108710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "200260",
    "title": "Batman: Arkham City GOTY",
    "releaseDate": "2012-09-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/200260/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/200260/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "200260",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "201510",
    "title": "FlatOut 3: Chaos & Destruction",
    "releaseDate": "2011-12-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/201510/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/201510/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "201510",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "204030",
    "title": "Fable - The Lost Chapters",
    "releaseDate": "2011-12-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/204030/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/204030/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "204030",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "204340",
    "title": "Serious Sam 2",
    "releaseDate": "2012-01-31",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/204340/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/204340/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "204340",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "208400",
    "title": "Avernum: Escape From the Pit",
    "releaseDate": "2012-04-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/208400/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/208400/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "208400",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "208580",
    "title": "STAR WARS™ Knights of the Old Republic™ II: The Sith Lords™",
    "releaseDate": "2012-08-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/208580/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/208580/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "208580",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "208650",
    "title": "Batman™: Arkham Knight",
    "releaseDate": "2015-06-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/208650/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/208650/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "208650",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "211820",
    "title": "Starbound",
    "releaseDate": "2016-07-22",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/211820/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/211820/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "211820",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "212680",
    "title": "FTL: Faster Than Light",
    "releaseDate": "2012-09-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/212680/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/212680/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "212680",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "220200",
    "title": "Kerbal Space Program",
    "releaseDate": "2015-04-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/220200/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/220200/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "220200",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "220240",
    "title": "Far Cry® 3",
    "releaseDate": "2012-12-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/220240/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/220240/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "220240",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "221100",
    "title": "DayZ",
    "releaseDate": "2018-12-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/221100/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/221100/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "221100",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "223830",
    "title": "Xenonauts",
    "releaseDate": "2014-06-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/223830/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/223830/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "223830",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "225540",
    "title": "Just Cause 3",
    "releaseDate": "2015-11-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/225540/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/225540/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "225540",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "227300",
    "title": "Euro Truck Simulator 2",
    "releaseDate": "2012-10-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/227300/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/227300/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "227300",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "230070",
    "title": "The Age of Decadence",
    "releaseDate": "2015-10-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/230070/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/230070/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "230070",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "230230",
    "title": "Divinity: Original Sin (Classic)",
    "releaseDate": "2014-06-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/230230/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/230230/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "230230",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "232430",
    "title": "Gone Home",
    "releaseDate": "2013-08-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/232430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/232430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "232430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "233860",
    "title": "Kenshi",
    "releaseDate": "2018-12-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/233860/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/233860/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "233860",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "237990",
    "title": "The Banner Saga",
    "releaseDate": "2014-01-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/237990/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/237990/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "237990",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "238010",
    "title": "Deus Ex: Human Revolution - Director's Cut",
    "releaseDate": "2013-10-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/238010/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/238010/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "238010",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "238320",
    "title": "Outlast",
    "releaseDate": "2013-09-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/238320/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/238320/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "238320",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "239140",
    "title": "Dying Light",
    "releaseDate": "2015-01-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/239140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/239140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "239140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "239820",
    "title": "Game Dev Tycoon",
    "releaseDate": "2013-08-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/239820/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/239820/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "239820",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "240760",
    "title": "Wasteland 2",
    "releaseDate": "2014-09-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/240760/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/240760/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "240760",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "242760",
    "title": "The Forest",
    "releaseDate": "2018-04-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/242760/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/242760/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "242760",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "242920",
    "title": "Banished",
    "releaseDate": "2014-02-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/242920/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/242920/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "242920",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "244850",
    "title": "Space Engineers",
    "releaseDate": "2019-02-28",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/244850/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/244850/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "244850",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "250520",
    "title": "Underrail",
    "releaseDate": "2015-12-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/250520/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/250520/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "250520",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "253110",
    "title": "The Cat Lady",
    "releaseDate": "2013-12-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/253110/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/253110/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "253110",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "255710",
    "title": "Cities: Skylines",
    "releaseDate": "2015-03-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/255710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/255710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "255710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "258180",
    "title": "Deus Ex: The Fall",
    "releaseDate": "2014-03-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/258180/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/258180/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "258180",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "261550",
    "title": "Mount & Blade II: Bannerlord",
    "releaseDate": "2022-10-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/261550/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/261550/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "261550",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "262060",
    "title": "Darkest Dungeon®",
    "releaseDate": "2016-01-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/262060/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/262060/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "262060",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "264710",
    "title": "Subnautica",
    "releaseDate": "2018-01-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/264710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/264710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "264710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "268050",
    "title": "The Evil Within",
    "releaseDate": "2014-10-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/268050/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/268050/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "268050",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "268500",
    "title": "XCOM 2",
    "releaseDate": "2016-02-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/268500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/268500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "268500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "272270",
    "title": "Torment: Tides of Numenera",
    "releaseDate": "2017-02-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/272270/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/272270/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "272270",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "274520",
    "title": "Darkwood",
    "releaseDate": "2017-08-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/274520/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/274520/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "274520",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "275850",
    "title": "No Man's Sky",
    "releaseDate": "2016-08-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/275850/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/275850/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "275850",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "281640",
    "title": "The Banner Saga 2",
    "releaseDate": "2016-04-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/281640/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/281640/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "281640",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "282140",
    "title": "SOMA",
    "releaseDate": "2015-09-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/282140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/282140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "282140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "286160",
    "title": "Tabletop Simulator",
    "releaseDate": "2015-06-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/286160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/286160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "286160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "286690",
    "title": "Metro 2033 Redux",
    "releaseDate": "2014-08-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/286690/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/286690/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "286690",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "287700",
    "title": "METAL GEAR SOLID V: THE PHANTOM PAIN",
    "releaseDate": "2015-09-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/287700/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/287700/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "287700",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "289130",
    "title": "ENDLESS Legend™",
    "releaseDate": "2014-09-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/289130/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/289130/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "289130",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "291650",
    "title": "Pillars of Eternity",
    "releaseDate": "2015-03-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/291650/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/291650/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "291650",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "292030",
    "title": "Ведьмак 3: Дикая Охота",
    "releaseDate": "2015-05-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/292030/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/292030/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "292030",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "294100",
    "title": "RimWorld",
    "releaseDate": "2018-10-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/294100/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/294100/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "294100",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "304650",
    "title": "Sunless Sea",
    "releaseDate": "2015-02-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/304650/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/304650/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "304650",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "306130",
    "title": "The Elder Scrolls Online",
    "releaseDate": "2017-05-22",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/306130/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/306130/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "306130",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "307690",
    "title": "Sleeping Dogs: Definitive Edition",
    "releaseDate": "2014-10-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/307690/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/307690/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "307690",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "323190",
    "title": "Frostpunk",
    "releaseDate": "2018-04-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/323190/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/323190/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "323190",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "335300",
    "title": "DARK SOULS™ II: Scholar of the First Sin",
    "releaseDate": "2015-04-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/335300/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/335300/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "335300",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "337000",
    "title": "Deus Ex: Mankind Divided™",
    "releaseDate": "2016-08-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/337000/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/337000/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "337000",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "346110",
    "title": "ARK: Survival Evolved",
    "releaseDate": "2017-08-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/346110/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/346110/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "346110",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "356190",
    "title": "Middle-earth™: Shadow of War™",
    "releaseDate": "2017-10-09",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/356190/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/356190/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "356190",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "359320",
    "title": "Elite Dangerous",
    "releaseDate": "2015-04-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/359320/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/359320/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "359320",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "361420",
    "title": "ASTRONEER",
    "releaseDate": "2019-02-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/361420/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/361420/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "361420",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "362890",
    "title": "Black Mesa",
    "releaseDate": "2020-03-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/362890/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/362890/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "362890",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "362960",
    "title": "Tyranny",
    "releaseDate": "2016-11-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/362960/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/362960/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "362960",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "365360",
    "title": "Battle Brothers",
    "releaseDate": "2017-03-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/365360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/365360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "365360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "367520",
    "title": "Hollow Knight",
    "releaseDate": "2017-02-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/367520/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/367520/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "367520",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "367540",
    "title": "Starbound - Unstable",
    "releaseDate": "",
    "releaseDateStatus": "unknown",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/367540/",
    "addedAt": "2026-06-22",
    "steamAppId": "367540",
    "sources": [
      "https://store.steampowered.com/app/367540/"
    ],
    "releaseDatePrecision": "unknown",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "368370",
    "title": "Her Story",
    "releaseDate": "2015-06-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/368370/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/368370/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "368370",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "371970",
    "title": "Barony",
    "releaseDate": "2015-06-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/371970/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/371970/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "371970",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "373420",
    "title": "Divinity: Original Sin Enhanced Edition",
    "releaseDate": "2015-10-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/373420/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/373420/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "373420",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "374320",
    "title": "DARK SOULS™ III",
    "releaseDate": "2016-04-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/374320/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/374320/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "374320",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "377160",
    "title": "Fallout 4",
    "releaseDate": "2015-11-09",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/377160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/377160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "377160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "379430",
    "title": "Kingdom Come: Deliverance",
    "releaseDate": "2018-02-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/379430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/379430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "379430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "379720",
    "title": "DOOM",
    "releaseDate": "2016-05-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/379720/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/379720/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "379720",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "383150",
    "title": "Dead Island Definitive Edition",
    "releaseDate": "2016-05-31",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/383150/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/383150/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "383150",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "392160",
    "title": "X4: Foundations",
    "releaseDate": "2018-11-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/392160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/392160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "392160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "394310",
    "title": "Punch Club",
    "releaseDate": "2016-01-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/394310/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/394310/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "394310",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "394510",
    "title": "HELLDIVERS™",
    "releaseDate": "2015-12-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/394510/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/394510/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "394510",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "396480",
    "title": "Battlevoid: Harbinger",
    "releaseDate": "2016-02-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/396480/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/396480/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "396480",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "404730",
    "title": "Wasteland 2: Director's Cut",
    "releaseDate": "2014-09-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/404730/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/404730/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "404730",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "407530",
    "title": "ARK: Survival Of The Fittest",
    "releaseDate": "2017-08-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/407530/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/407530/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "407530",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "412020",
    "title": "Metro Exodus",
    "releaseDate": "2020-02-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/412020/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/412020/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "412020",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "413150",
    "title": "Stardew Valley",
    "releaseDate": "2016-02-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/413150/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/413150/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "413150",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "414700",
    "title": "Outlast 2",
    "releaseDate": "2017-04-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/414700/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/414700/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "414700",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "418370",
    "title": "Resident Evil 7 Biohazard",
    "releaseDate": "2017-01-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/418370/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/418370/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "418370",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "428660",
    "title": "Deliver Us The Moon",
    "releaseDate": "2019-10-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/428660/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/428660/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "428660",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "431960",
    "title": "Wallpaper Engine",
    "releaseDate": "2018-11-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/431960/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/431960/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "431960",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "435150",
    "title": "Divinity: Original Sin 2",
    "releaseDate": "2017-09-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/435150/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/435150/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "435150",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "438100",
    "title": "VRChat",
    "releaseDate": "2017-02-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2017-02-01",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/438100/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/438100/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "438100",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "438420",
    "title": "Zenith",
    "releaseDate": "2016-09-20",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/438420/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/438420/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "438420",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "447040",
    "title": "Watch_Dogs 2",
    "releaseDate": "2016-11-28",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/447040/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/447040/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "447040",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "457140",
    "title": "Oxygen Not Included",
    "releaseDate": "2019-07-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/457140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/457140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "457140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "466300",
    "title": "Planescape: Torment: Enhanced Edition",
    "releaseDate": "2017-04-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/466300/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/466300/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "466300",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "471710",
    "title": "Rec Room",
    "releaseDate": "2021-09-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/471710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/471710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "471710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "474960",
    "title": "Quantum Break",
    "releaseDate": "2016-09-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/474960/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/474960/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "474960",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "477160",
    "title": "Human Fall Flat",
    "releaseDate": "2016-07-22",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/477160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/477160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "477160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "480490",
    "title": "Prey",
    "releaseDate": "2017-05-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/480490/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/480490/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "480490",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "485460",
    "title": "The Banner Saga 3",
    "releaseDate": "2018-07-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/485460/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/485460/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "485460",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "489630",
    "title": "Warhammer 40,000: Gladius - Relics of War",
    "releaseDate": "2018-07-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/489630/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/489630/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "489630",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "489830",
    "title": "The Elder Scrolls V: Skyrim Special Edition",
    "releaseDate": "2016-10-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/489830/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/489830/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "489830",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "493490",
    "title": "City Car Driving",
    "releaseDate": "2016-11-03",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/493490/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/493490/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "493490",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "505230",
    "title": "Pathologic 2",
    "releaseDate": "2019-05-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/505230/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/505230/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "505230",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "515960",
    "title": "Martha Is Dead",
    "releaseDate": "2022-02-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/515960/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/515960/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "515960",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "526870",
    "title": "Satisfactory",
    "releaseDate": "2024-09-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/526870/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/526870/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "526870",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "534380",
    "title": "Dying Light 2: Reloaded Edition",
    "releaseDate": "2022-02-03",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/534380/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/534380/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "534380",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "538030",
    "title": "Xenonauts 2",
    "releaseDate": "2026-04-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/538030/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/538030/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "538030",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "544610",
    "title": "Battlestar Galactica Deadlock",
    "releaseDate": "2017-08-31",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/544610/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/544610/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "544610",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "546560",
    "title": "Half-Life: Alyx",
    "releaseDate": "2020-03-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/546560/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/546560/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "546560",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "548430",
    "title": "Deep Rock Galactic",
    "releaseDate": "2020-05-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/548430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/548430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "548430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "552500",
    "title": "Warhammer: Vermintide 2",
    "releaseDate": "2018-03-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/552500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/552500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "552500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "552620",
    "title": "ATOM RPG",
    "releaseDate": "2018-12-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/552620/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/552620/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "552620",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "560130",
    "title": "Pillars of Eternity II: Deadfire",
    "releaseDate": "2018-05-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/560130/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/560130/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "560130",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "564310",
    "title": "Serious Sam Fusion 2017 (beta)",
    "releaseDate": "2017-03-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/564310/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/564310/",
      "https://serioussam.fandom.com/wiki/Serious_Sam_Fusion_2017"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "564310",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "570940",
    "title": "DARK SOULS™: REMASTERED",
    "releaseDate": "2018-05-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/570940/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/570940/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "570940",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "578080",
    "title": "PUBG: BATTLEGROUNDS",
    "releaseDate": "2017-12-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/578080/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/578080/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "578080",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "588430",
    "title": "Fallout Shelter",
    "releaseDate": "2017-03-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/588430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/588430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "588430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "590380",
    "title": "Into the Breach",
    "releaseDate": "2018-02-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/590380/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/590380/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "590380",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "599140",
    "title": "Graveyard Keeper",
    "releaseDate": "2018-08-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/599140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/599140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "599140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "601150",
    "title": "Devil May Cry 5",
    "releaseDate": "2019-03-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/601150/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/601150/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "601150",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "601430",
    "title": "The Evil Within 2",
    "releaseDate": "2017-10-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/601430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/601430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "601430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "606160",
    "title": "ROUTINE",
    "releaseDate": "2025-12-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/606160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/606160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "606160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "611670",
    "title": "The Elder Scrolls V: Skyrim VR",
    "releaseDate": "2018-04-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/611670/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/611670/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "611670",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "613100",
    "title": "Хаус Флиппер",
    "releaseDate": "2018-05-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/613100/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/613100/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "613100",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "617290",
    "title": "Remnant: From the Ashes",
    "releaseDate": "2019-08-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/617290/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/617290/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "617290",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "617830",
    "title": "SUPERHOT VR",
    "releaseDate": "2017-05-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/617830/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/617830/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "617830",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "619700",
    "title": "Steel Rats",
    "releaseDate": "2018-11-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/619700/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/619700/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "619700",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "620980",
    "title": "Beat Saber",
    "releaseDate": "2019-05-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/620980/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/620980/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "620980",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "632470",
    "title": "Disco Elysium",
    "releaseDate": "2019-10-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/632470/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/632470/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "632470",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "638230",
    "title": "Journey",
    "releaseDate": "2020-06-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/638230/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/638230/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "638230",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "639770",
    "title": "UNTITLED",
    "releaseDate": "2017-06-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/639770/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/639770/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "639770",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "640820",
    "title": "Pathfinder: Kingmaker",
    "releaseDate": "2018-09-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/640820/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/640820/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "640820",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "645630",
    "title": "Car Mechanic Simulator 2018",
    "releaseDate": "2017-07-28",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/645630/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/645630/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "645630",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "648410",
    "title": "Colony Ship: A Post-Earth Role Playing Game",
    "releaseDate": "2023-11-09",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/648410/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/648410/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "648410",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "660160",
    "title": "Field of Glory II",
    "releaseDate": "2017-10-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/660160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/660160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "660160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "673880",
    "title": "Warhammer 40,000: Mechanicus",
    "releaseDate": "2018-11-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/673880/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/673880/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "673880",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "681280",
    "title": "Descenders",
    "releaseDate": "2019-05-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/681280/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/681280/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "681280",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "698670",
    "title": "Scorn",
    "releaseDate": "2022-10-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/698670/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/698670/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "698670",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "716630",
    "title": "Battlevoid: Sector Siege",
    "releaseDate": "2017-10-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/716630/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/716630/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "716630",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "719040",
    "title": "Wasteland 3",
    "releaseDate": "2020-08-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/719040/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/719040/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "719040",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "731040",
    "title": "The Invincible",
    "releaseDate": "2023-11-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/731040/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/731040/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "731040",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "738520",
    "title": "Breathedge",
    "releaseDate": "2021-02-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/738520/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/738520/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "738520",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "741820",
    "title": "Prey: Typhon Hunter",
    "releaseDate": "2018-12-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/741820/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/741820/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "741820",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "752590",
    "title": "A Plague Tale: Innocence",
    "releaseDate": "2019-05-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/752590/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/752590/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "752590",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "753640",
    "title": "Outer Wilds",
    "releaseDate": "2020-06-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/753640/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/753640/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "753640",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "782330",
    "title": "DOOM Eternal",
    "releaseDate": "2020-03-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/782330/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/782330/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "782330",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "799600",
    "title": "Cosmoteer: Конструктор и командир звездолёта",
    "releaseDate": "2022-10-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2022-10-24",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/799600/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/799600/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "799600",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "805550",
    "title": "Assetto Corsa Competizione",
    "releaseDate": "2019-05-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/805550/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/805550/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "805550",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "814380",
    "title": "Sekiro™: Shadows Die Twice",
    "releaseDate": "2019-03-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/814380/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/814380/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "814380",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "835960",
    "title": "The Talos Principle 2",
    "releaseDate": "2023-11-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/835960/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/835960/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "835960",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "848450",
    "title": "Subnautica: Below Zero",
    "releaseDate": "2021-05-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/848450/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/848450/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "848450",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "865680",
    "title": "The Eternal Cylinder",
    "releaseDate": "2022-10-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/865680/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/865680/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "865680",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "870780",
    "title": "Control Ultimate Edition",
    "releaseDate": "2020-08-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/870780/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/870780/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "870780",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "873940",
    "title": "Crying Suns",
    "releaseDate": "2019-09-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/873940/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/873940/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "873940",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "883710",
    "title": "Resident Evil 2",
    "releaseDate": "2019-01-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/883710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/883710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "883710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "892970",
    "title": "Valheim",
    "releaseDate": "2021-02-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2021-02-02",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/892970/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/892970/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "892970",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "906100",
    "title": "Observation",
    "releaseDate": "2020-05-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/906100/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/906100/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "906100",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "920210",
    "title": "LEGO® Звездные Войны™: Скайуокер. Сага",
    "releaseDate": "2022-04-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/920210/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/920210/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "920210",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "933820",
    "title": "Endzone - A World Apart",
    "releaseDate": "2021-03-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/933820/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/933820/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "933820",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "939850",
    "title": "The Dark Pictures Anthology: Man of Medan",
    "releaseDate": "2019-08-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/939850/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/939850/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "939850",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "941460",
    "title": "Death Trash",
    "releaseDate": "2021-08-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2021-08-05",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/941460/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/941460/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "941460",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "945360",
    "title": "Among Us",
    "releaseDate": "2018-11-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/945360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/945360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "945360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "952060",
    "title": "Resident Evil 3",
    "releaseDate": "2020-04-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/952060/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/952060/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "952060",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "952070",
    "title": "RESIDENT EVIL RESISTANCE",
    "releaseDate": "2020-04-03",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/952070/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/952070/",
      "https://www.residentevil.com/resistance/us/news/obt.html"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "952070",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "954740",
    "title": "Terminator: Resistance",
    "releaseDate": "2019-11-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/954740/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/954740/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "954740",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "960910",
    "title": "Heavy Rain",
    "releaseDate": "2020-06-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/960910/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/960910/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "960910",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "960990",
    "title": "Beyond: Two Souls",
    "releaseDate": "2020-06-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/960990/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/960990/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "960990",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "975370",
    "title": "Dwarf Fortress",
    "releaseDate": "2022-12-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/975370/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/975370/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "975370",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "976620",
    "title": "Enderal: Forgotten Stories (Special Edition)",
    "releaseDate": "2021-04-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/976620/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/976620/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "976620",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "979110",
    "title": "Space Haven",
    "releaseDate": "2026-05-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/979110/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/979110/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "979110",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "999220",
    "title": "Amnesia: Rebirth",
    "releaseDate": "2020-10-20",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/999220/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/999220/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "999220",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1030840",
    "title": "Mafia: Definitive Edition",
    "releaseDate": "2020-09-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1030840/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1030840/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1030840",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1080020",
    "title": "Electrician Simulator",
    "releaseDate": "2022-09-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1080020/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1080020/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1080020",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1084160",
    "title": "Jagged Alliance 3",
    "releaseDate": "2023-07-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1084160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1084160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1084160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1086940",
    "title": "Baldur's Gate 3",
    "releaseDate": "2023-08-03",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1086940/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1086940/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1086940",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1088850",
    "title": "Marvel's Guardians of the Galaxy",
    "releaseDate": "2021-10-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1088850/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1088850/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1088850",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1101360",
    "title": "Rock of Ages 3: Make & Break",
    "releaseDate": "2020-07-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1101360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1101360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1101360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1103210",
    "title": "We Become What We Behold",
    "releaseDate": "2019-07-09",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1103210/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1103210/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1103210",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1129580",
    "title": "Medieval Dynasty",
    "releaseDate": "2021-09-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1129580/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1129580/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1129580",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1133870",
    "title": "Space Engineers 2",
    "releaseDate": "2025-01-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2025-01-27",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/1133870/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1133870/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1133870",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1138660",
    "title": "Черная Книга",
    "releaseDate": "2021-08-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1138660/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1138660/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1138660",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1139940",
    "title": "ATOM RPG Trudograd",
    "releaseDate": "2021-09-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1139940/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1139940/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1139940",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1145360",
    "title": "Hades",
    "releaseDate": "2020-09-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1145360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1145360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1145360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1147550",
    "title": "Not For Broadcast",
    "releaseDate": "2022-01-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1147550/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1147550/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1147550",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1151340",
    "title": "Fallout 76",
    "releaseDate": "2020-04-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1151340/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1151340/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1151340",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1151640",
    "title": "Horizon Zero Dawn™ Complete Edition",
    "releaseDate": "2020-08-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1151640/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1151640/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1151640",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1158310",
    "title": "Crusader Kings III",
    "releaseDate": "2020-09-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1158310/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1158310/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1158310",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1161580",
    "title": "Hardspace: Shipbreaker",
    "releaseDate": "2022-05-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1161580/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1161580/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1161580",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1161590",
    "title": "Punch Club 2: Fast Forward",
    "releaseDate": "2023-07-20",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1161590/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1161590/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1161590",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1167630",
    "title": "Teardown",
    "releaseDate": "2022-04-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1167630/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1167630/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1167630",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1172380",
    "title": "ЗВЁЗДНЫЕ ВОЙНЫ Джедаи: Павший Орден™",
    "releaseDate": "2019-11-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1172380/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1172380/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1172380",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1174180",
    "title": "Red Dead Redemption 2",
    "releaseDate": "2019-12-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1174180/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1174180/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1174180",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1176710",
    "title": "Space Crew: Legendary Edition",
    "releaseDate": "2020-10-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1176710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1176710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1176710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1182900",
    "title": "A Plague Tale: Requiem",
    "releaseDate": "2022-10-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1182900/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1182900/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1182900",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1184370",
    "title": "Pathfinder: Wrath of the Righteous - Enhanced Edition",
    "releaseDate": "2021-09-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1184370/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1184370/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1184370",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1194630",
    "title": "The Dark Pictures Anthology: Little Hope",
    "releaseDate": "2020-10-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1194630/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1194630/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1194630",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1196590",
    "title": "Resident Evil Village",
    "releaseDate": "2021-05-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1196590/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1196590/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1196590",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1199030",
    "title": "Tainted Grail",
    "releaseDate": "2021-05-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1199030/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1199030/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1199030",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1222140",
    "title": "Detroit: Become Human",
    "releaseDate": "2020-06-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1222140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1222140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1222140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1222680",
    "title": "Need for Speed™ Heat",
    "releaseDate": "2020-06-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1222680/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1222680/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1222680",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1222700",
    "title": "A Way Out",
    "releaseDate": "2020-06-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1222700/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1222700/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1222700",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1222730",
    "title": "STAR WARS™: Squadrons",
    "releaseDate": "2020-10-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1222730/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1222730/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1222730",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1235140",
    "title": "Yakuza: Like a Dragon",
    "releaseDate": "2020-11-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1235140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1235140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1235140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1236300",
    "title": "Resident Evil Re:Verse",
    "releaseDate": "2022-10-20",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1236300/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1236300/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1236300",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1237950",
    "title": "STAR WARS™ Battlefront™ II",
    "releaseDate": "2020-06-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1237950/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1237950/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1237950",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1237970",
    "title": "Titanfall® 2",
    "releaseDate": "2020-06-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1237970/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1237970/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1237970",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1245430",
    "title": "Golden Light",
    "releaseDate": "2022-03-11",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1245430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1245430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1245430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1245620",
    "title": "ELDEN RING",
    "releaseDate": "2022-02-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1245620/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1245620/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1245620",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1259420",
    "title": "Days Gone",
    "releaseDate": "2021-05-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1259420/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1259420/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1259420",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1272160",
    "title": "The Life and Suffering of Sir Brante",
    "releaseDate": "2021-03-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1272160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1272160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1272160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1281590",
    "title": "The Dark Pictures Anthology: House of Ashes",
    "releaseDate": "2021-10-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1281590/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1281590/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1281590",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1284190",
    "title": "The Planet Crafter",
    "releaseDate": "2024-04-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1284190/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1284190/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1284190",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1297900",
    "title": "Gothic 1 Remake",
    "releaseDate": "2026-06-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1297900/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1297900/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1297900",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1304930",
    "title": "The Outlast Trials",
    "releaseDate": "2024-03-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1304930/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1304930/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1304930",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1307710",
    "title": "GRID Legends",
    "releaseDate": "2022-02-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1307710/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1307710/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1307710",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1326470",
    "title": "Sons Of The Forest",
    "releaseDate": "2024-02-22",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1326470/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1326470/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1326470",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1328660",
    "title": "Need for Speed™ Hot Pursuit Remastered",
    "releaseDate": "2020-11-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1328660/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1328660/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1328660",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1328670",
    "title": "Mass Effect™ издание Legendary",
    "releaseDate": "2021-05-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1328670/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1328670/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1328670",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1332010",
    "title": "Stray",
    "releaseDate": "2022-07-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1332010/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1332010/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1332010",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1336490",
    "title": "Against the Storm",
    "releaseDate": "2023-12-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1336490/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1336490/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1336490",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1367590",
    "title": "Tormented Souls",
    "releaseDate": "2021-08-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1367590/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1367590/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1367590",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1371980",
    "title": "No Rest for the Wicked",
    "releaseDate": "2024-04-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2024-04-18",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/1371980/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1371980/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1371980",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1373960",
    "title": "INDIKA",
    "releaseDate": "2024-05-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1373960/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1373960/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1373960",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1376200",
    "title": "KARMA: The Dark World",
    "releaseDate": "2025-03-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1376200/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1376200/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1376200",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1386900",
    "title": "Observer: System Redux",
    "releaseDate": "2020-11-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1386900/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1386900/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1386900",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1422450",
    "title": "Deadlock",
    "releaseDate": "",
    "releaseDateStatus": "unknown",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1422450/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1422450/"
    ],
    "releaseDatePrecision": "unknown",
    "steamAppId": "1422450",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1426210",
    "title": "It Takes Two",
    "releaseDate": "2021-03-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1426210/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1426210/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1426210",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1449560",
    "title": "Metro Exodus Enhanced Edition",
    "releaseDate": "2020-02-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1449560/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1449560/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1449560",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1458140",
    "title": "Pacific Drive",
    "releaseDate": "2024-02-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1458140/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1458140/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1458140",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1469610",
    "title": "Last Train Home",
    "releaseDate": "2023-11-28",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1469610/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1469610/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1469610",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1481400",
    "title": "Dagon",
    "releaseDate": "2021-09-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1481400/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1481400/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1481400",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1527950",
    "title": "Wartales",
    "releaseDate": "2023-04-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1527950/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1527950/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1527950",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1537710",
    "title": "The Dark Pictures Anthology: Little Hope - Friend's Pass",
    "releaseDate": "",
    "releaseDateStatus": "unknown",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1537710/",
    "addedAt": "2026-06-22",
    "steamAppId": "1537710",
    "sources": [
      "https://store.steampowered.com/app/1537710/"
    ],
    "releaseDatePrecision": "unknown",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1551360",
    "title": "Forza Horizon 5",
    "releaseDate": "2021-11-08",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1551360/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1551360/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1551360",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1562430",
    "title": "DREDGE",
    "releaseDate": "2023-03-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1562430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1562430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1562430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1583230",
    "title": "High On Life",
    "releaseDate": "2022-12-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1583230/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1583230/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1583230",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1593500",
    "title": "God of War",
    "releaseDate": "2022-01-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1593500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1593500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1593500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1601570",
    "title": "The Alters",
    "releaseDate": "2025-06-13",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1601570/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1601570/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1601570",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1601580",
    "title": "Frostpunk 2",
    "releaseDate": "2024-09-20",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1601580/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1601580/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1601580",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1609080",
    "title": "Summer of '58",
    "releaseDate": "2021-07-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1609080/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1609080/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1609080",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1620040",
    "title": "Nickelodeon Kart Racers 3: Slime Speedway",
    "releaseDate": "2022-10-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1620040/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1620040/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1620040",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1649240",
    "title": "Returnal™",
    "releaseDate": "2023-02-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1649240/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1649240/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1649240",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1659040",
    "title": "HITMAN World of Assassination",
    "releaseDate": "2022-01-20",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1659040/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1659040/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1659040",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1669000",
    "title": "Age of Wonders 4",
    "releaseDate": "2023-05-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1669000/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1669000/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1669000",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1681430",
    "title": "RoboCop: Rogue City",
    "releaseDate": "2023-11-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1681430/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1681430/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1681430",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1684350",
    "title": "The Thaumaturge",
    "releaseDate": "2024-03-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1684350/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1684350/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1684350",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1693980",
    "title": "Dead Space",
    "releaseDate": "2023-01-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1693980/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1693980/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1693980",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1716740",
    "title": "Starfield",
    "releaseDate": "2023-09-05",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1716740/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1716740/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1716740",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1771300",
    "title": "Kingdom Come: Deliverance II",
    "releaseDate": "2025-02-04",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1771300/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1771300/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1771300",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1774580",
    "title": "STAR WARS Jedi: Survivor™",
    "releaseDate": "2023-04-27",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1774580/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1774580/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1774580",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1775270",
    "title": "The Dark Pictures Anthology: House of Ashes - Friend's Pass",
    "releaseDate": "2022-01-28",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1775270/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1775270/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1775270",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1783560",
    "title": "The Last Caretaker",
    "releaseDate": "2025-11-06",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2025-11-06",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/1783560/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1783560/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1783560",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1792250",
    "title": "Serious Sam: Siberian Mayhem",
    "releaseDate": "2022-01-25",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1792250/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1792250/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1792250",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1808500",
    "title": "ARC Raiders",
    "releaseDate": "2025-10-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1808500/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1808500/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1808500",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1830720",
    "title": "MOTHERED - A ROLE-PLAYING HORROR GAME",
    "releaseDate": "2021-12-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1830720/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1830720/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1830720",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1857080",
    "title": "The Last Starship",
    "releaseDate": "2026-02-03",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1857080/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1857080/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1857080",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1903340",
    "title": "Clair Obscur: Expedition 33",
    "releaseDate": "2025-04-24",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1903340/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1903340/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1903340",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1946700",
    "title": "Layers of Fear",
    "releaseDate": "2023-06-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/1946700/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1946700/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1946700",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "1962700",
    "title": "Subnautica 2",
    "releaseDate": "2026-05-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2026-05-14",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/1962700/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/1962700/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "1962700",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2050650",
    "title": "Resident Evil 4",
    "releaseDate": "2023-03-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2050650/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2050650/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2050650",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2274200",
    "title": "Arken Age",
    "releaseDate": "2025-01-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2274200/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2274200/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2274200",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2362420",
    "title": "Mass Effect 2 (2010) Edition",
    "releaseDate": "2023-05-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2362420/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2362420/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2362420",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2464280",
    "title": "Tormented Souls 2",
    "releaseDate": "2025-10-23",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2464280/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2464280/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2464280",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2475490",
    "title": "Mouthwashing",
    "releaseDate": "2024-09-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2475490/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2475490/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2475490",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2497900",
    "title": "DON'T SCREAM",
    "releaseDate": "2024-10-28",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2497900/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2497900/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2497900",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2592160",
    "title": "Dispatch",
    "releaseDate": "2025-10-22",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2592160/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2592160/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2592160",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2629330",
    "title": "МЕМОЛОГИЯ: ЕГЭ ПО МЕМАМ",
    "releaseDate": "2024-05-15",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2629330/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2629330/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2629330",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2806640",
    "title": "The Talos Principle: Reawakened",
    "releaseDate": "2025-04-10",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2806640/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2806640/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2806640",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2842040",
    "title": "Star Wars Outlaws",
    "releaseDate": "2024-11-21",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2842040/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2842040/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2842040",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2859370",
    "title": "Игра «Жизнь»: время",
    "releaseDate": "2024-03-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2859370/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2859370/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2859370",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2881650",
    "title": "Content Warning",
    "releaseDate": "2024-04-01",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2881650/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2881650/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2881650",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "2967990",
    "title": "Train Sim World® 5",
    "releaseDate": "2024-09-17",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/2967990/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/2967990/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "2967990",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3008130",
    "title": "Dying Light: The Beast",
    "releaseDate": "2025-09-18",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3008130/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3008130/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3008130",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3097560",
    "title": "Liar's Bar",
    "releaseDate": "2024-10-02",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2024-10-02",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/3097560/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3097560/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3097560",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3105440",
    "title": "Heroes of Might and Magic: Olden Era",
    "releaseDate": "2026-04-30",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "2026-04-30",
    "earlyAccessDateStatus": "exact",
    "sourceUrl": "https://store.steampowered.com/app/3105440/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3105440/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3105440",
    "earlyAccessDatePrecision": "day",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3167020",
    "title": "Escape from Duckov",
    "releaseDate": "2025-10-16",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3167020/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3167020/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3167020",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3244220",
    "title": "A Game About Digging A Hole™",
    "releaseDate": "2025-02-07",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3244220/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3244220/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3244220",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3280350",
    "title": "DEATH STRANDING 2: ON THE BEACH",
    "releaseDate": "2026-03-19",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3280350/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3280350/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3280350",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3419520",
    "title": "Quarantine Zone: The Last Check",
    "releaseDate": "2026-01-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3419520/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3419520/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3419520",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3564740",
    "title": "Where Winds Meet",
    "releaseDate": "2025-11-14",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3564740/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3564740/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3564740",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3764200",
    "title": "Resident Evil Requiem",
    "releaseDate": "2026-02-26",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3764200/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3764200/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3764200",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3829120",
    "title": "NextVisuals Benchmark",
    "releaseDate": "2025-07-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3829120/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3829120/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3829120",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3922100",
    "title": "Forest Doesn’t Care",
    "releaseDate": "2025-08-29",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3922100/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3922100/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3922100",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  },
  {
    "id": "3929970",
    "title": "Cave Crave VR",
    "releaseDate": "2025-12-12",
    "releaseDateStatus": "exact",
    "earlyAccessDate": "",
    "earlyAccessDateStatus": "unknown",
    "sourceUrl": "https://store.steampowered.com/app/3929970/",
    "addedAt": "2026-06-22",
    "sources": [
      "https://store.steampowered.com/app/3929970/"
    ],
    "releaseDatePrecision": "day",
    "steamAppId": "3929970",
    "earlyAccessDatePrecision": "unknown",
    "platform": "Steam",
    "status": ""
  }
]
const LS_DATA='gameLibraryData.v1', LS_PREF='gameLibraryPrefs.v1', LS_BACKUP='gameLibraryData.backup.v1', DATA_VERSION=2;
const PLATFORMS=['','Steam','Epic','GOG','PS','Пиратка','Другое'];
const STATUSES=['','Прошёл','Дропнул'];
const DEFAULT_PREFS={sort:'title',dir:'asc',view:'table',theme:'dark',q:'',hasEa:false,unknown:false,decade:'',platform:'',statusTab:'all'};
let lastMigrationSteamCount=0;
let loaded=loadData(), games=loaded.games, prefs=loadPrefs();
if(loaded.migrated)save();
if(prefs.status&&!prefs.statusTab)prefs.statusTab=prefs.status==='__empty__'?'empty':prefs.status;
delete prefs.status;
if(!['all','empty','Прошёл','Дропнул'].includes(prefs.statusTab))prefs.statusTab='all';
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
function clone(v){return JSON.parse(JSON.stringify(v))}
function validPlatform(v){return v==null?'':String(v)}
function validStatus(v){return STATUSES.includes(v)?v:''}
function platformClass(platform){return {'Steam':'platform-steam','Epic':'platform-epic','GOG':'platform-gog','PS':'platform-ps','Пиратка':'platform-pirate','Другое':'platform-other'}[platform]||'platform-empty'}
function normalizeGame(g){let x={...g}; let defaults={id:String(Date.now()+Math.random()),title:'',releaseDate:'',releaseDateStatus:'unknown',earlyAccessDate:'',earlyAccessDateStatus:'unknown',platform:'',status:'',sourceUrl:'',addedAt:new Date().toISOString().slice(0,10),sources:[]}; Object.keys(defaults).forEach(k=>{if(x[k]===undefined||x[k]===null)x[k]=defaults[k]}); x.platform=validPlatform(x.platform); x.status=validStatus(x.status); if(!Array.isArray(x.sources))x.sources=[]; return x}
function exportGame(g){let x=normalizeGame(g); Object.keys(x).forEach(k=>x[k]===undefined&&delete x[k]); return x}
function save(){localStorage.setItem(LS_DATA,JSON.stringify(games.map(exportGame)));}
function savePrefs(){localStorage.setItem(LS_PREF,JSON.stringify(prefs)); document.documentElement.dataset.theme=prefs.theme;}
function loadPrefs(){try{return Object.assign({},DEFAULT_PREFS,JSON.parse(localStorage.getItem(LS_PREF)||'{}'))}catch{return {...DEFAULT_PREFS}}}
function needsMigration(g){return ['id','title','releaseDate','releaseDateStatus','earlyAccessDate','earlyAccessDateStatus','platform','status','sourceUrl','addedAt','sources'].some(k=>g?.[k]===undefined||g?.[k]===null)||!Array.isArray(g?.sources)||!STATUSES.includes(g.status)}
function loadData(){let stored=localStorage.getItem(LS_DATA); if(stored!==null){try{let raw=JSON.parse(stored), arr=Array.isArray(raw)?raw:(Array.isArray(raw?.games)?raw.games:null); if(Array.isArray(arr)){let migrated=arr.some(needsMigration); if(migrated&&!localStorage.getItem(LS_BACKUP))localStorage.setItem(LS_BACKUP,stored); return {games:arr.map(normalizeGame),migrated}}}catch(e){console.error('Не удалось прочитать сохраненную библиотеку:',e); return {games:[],migrated:false}}} return {games:clone(ORIGINAL_GAMES).map(normalizeGame),migrated:false}}
function fmt(d,st){if(!d)return '—'; if(st==='year')return d.slice(0,4); return new Intl.DateTimeFormat('ru-RU',{day:'numeric',month:'long',year:'numeric'}).format(new Date(d+'T00:00:00'));}
function text(el,v){el.textContent=v||''} function unknown(g){return !g.releaseDate||g.releaseDateStatus==='unknown'||g.earlyAccessDateStatus==='unknown'}
function badge(v,type){let s=document.createElement('span'); s.className='badge '+(type||''); s.textContent=v||'—'; return s}
function ensureSelectOption(select,value){if(value&&![...select.children].some(o=>o.value===value)){let option=document.createElement('option'); option.value=value; option.textContent=value; select.append(option);}}
function createInlineSelect(game, field, options){let select=document.createElement('select'); select.className='inline-select'; select.setAttribute('aria-label',field==='platform'?'Платформа':'Статус'); let current=game[field]||'', list=[...options]; if(current&&!list.some(o=>o.value===current))list.push({value:current,label:current}); list.forEach(({value,label})=>{let option=document.createElement('option'); option.value=value; option.textContent=label; select.append(option);}); select.value=current; select.onchange=()=>{let next=field==='platform'?validPlatform(select.value):validStatus(select.value); if(game[field]===next)return; games=games.map(g=>g.id===game.id?{...g,[field]:next}:g); save(); render();}; return select}
const PLATFORM_OPTIONS=PLATFORMS.map(value=>({value,label:value||'не указана'}));
const STATUS_OPTIONS=STATUSES.map(value=>({value,label:value||'без статуса'}));
function matchesStatusTab(g){return prefs.statusTab==='all'||(prefs.statusTab==='empty'?!g.status:g.status===prefs.statusTab)}
function filtered(){let q=prefs.q.toLowerCase(); return games.filter(g=>(!q||g.title.toLowerCase().includes(q))&&(!prefs.hasEa||g.earlyAccessDate)&&(!prefs.unknown||unknown(g))&&(!prefs.decade||(g.releaseDate&&Math.floor(+g.releaseDate.slice(0,4)/10)*10==prefs.decade))&&(!prefs.platform||(prefs.platform==='__empty__'?!g.platform:g.platform===prefs.platform))&&matchesStatusTab(g)).sort(cmp)}
function cmp(a,b){let k=prefs.sort, av=a[k]||'', bv=b[k]||''; if(!av&&bv)return 1; if(av&&!bv)return -1; let r=k==='title'?av.localeCompare(bv,'ru',{sensitivity:'base'}):av.localeCompare(bv,'ru'); return prefs.dir==='asc'?r:-r;}
function render(){savePrefs(); $('#search').value=prefs.q; $('#hasEa').checked=prefs.hasEa; $('#unknown').checked=prefs.unknown; $('#decade').value=prefs.decade; $('#platformFilter').value=prefs.platform; $('#themeBtn').textContent=prefs.theme==='dark'?'Светлая тема':'Тёмная тема'; $('#viewBtn').textContent=prefs.view==='table'?'Карточки':'Таблица'; document.body.classList.toggle('card-mode',prefs.view==='cards'); renderDecades(); renderPlatformFilter(); renderStatusTabs(); let rows=filtered(); text($('#counter'),`Найдено ${rows.length} из ${games.length} игр`); renderStats(); renderTable(rows); renderCards(rows);}
function renderDecades(){let vals=[...new Set(games.filter(g=>g.releaseDate).map(g=>Math.floor(+g.releaseDate.slice(0,4)/10)*10))].sort(); $('#decade').innerHTML='<option value="">Все десятилетия</option>'+vals.map(v=>`<option value="${v}">${v}-е годы</option>`).join('')}
function renderPlatformFilter(){let used=[...new Set(games.map(g=>g.platform).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru')); $('#platformFilter').innerHTML='<option value="">Все платформы</option><option value="__empty__">Платформа не указана</option>'+used.map(v=>`<option value="${v}">${v}</option>`).join(''); $('#platformFilter').value=prefs.platform;}
function statusTabCounts(){return {all:games.length,empty:games.filter(g=>!g.status).length,'Прошёл':games.filter(g=>g.status==='Прошёл').length,'Дропнул':games.filter(g=>g.status==='Дропнул').length}}
function renderStatusTabs(){let box=$('#statusTabs'), counts=statusTabCounts(); box.textContent=''; [{value:'all',label:'Все'},{value:'empty',label:'Без статуса'},{value:'Прошёл',label:'Прошёл'},{value:'Дропнул',label:'Дропнул'}].forEach(tab=>{let b=document.createElement('button'); b.type='button'; b.className='status-tab'; b.classList.toggle('active',prefs.statusTab===tab.value); b.setAttribute('aria-pressed',String(prefs.statusTab===tab.value)); b.textContent=`${tab.label} ${counts[tab.value]}`; b.onclick=()=>{prefs.statusTab=tab.value; render()}; box.append(b);});}
function cells(tr, vals){vals.forEach(v=>{let td=document.createElement('td'); if(v instanceof Node)td.append(v); else td.textContent=v; tr.append(td);});}
function actions(g){let box=document.createElement('div'); box.className='row-actions'; let e=document.createElement('button'), d=document.createElement('button'); e.textContent='Изменить'; d.textContent='Удалить'; e.onclick=()=>openForm(g); d.onclick=()=>{if(confirm(`Удалить «${g.title}»?`)){games=games.filter(x=>x.id!==g.id); save(); render();}}; box.append(e,d); return box;}
function statusBadge(g){return badge(g.status,g.status==='Прошёл'?'ok':g.status==='Дропнул'?'warn':'')}
function renderTable(rows){$$('th[data-sort]').forEach(th=>{th.classList.toggle('active',th.dataset.sort===prefs.sort); th.textContent=({title:'Название',releaseDate:'Полный релиз',earlyAccessDate:'Ранний доступ',platform:'Платформа',status:'Статус'}[th.dataset.sort])+(th.dataset.sort===prefs.sort?(prefs.dir==='asc'?' ▲':' ▼'):'')}); let tb=$('#gamesTable tbody'); tb.textContent=''; rows.forEach(g=>{let tr=document.createElement('tr'); tr.classList.add(platformClass(g.platform)); cells(tr,[g.title,fmt(g.releaseDate,g.releaseDateStatus),fmt(g.earlyAccessDate,g.earlyAccessDateStatus),createInlineSelect(g,'platform',PLATFORM_OPTIONS),createInlineSelect(g,'status',STATUS_OPTIONS),actions(g)]); tb.append(tr);});}
function labeledInlineSelect(label, select){let box=document.createElement('div'); box.className='card-inline-field'; let caption=document.createElement('span'); caption.textContent=label; box.append(caption,select); return box}
function renderCards(rows){let c=$('#cards'); c.textContent=''; rows.forEach(g=>{let a=document.createElement('article'); a.className='card '+platformClass(g.platform); cells(a,[g.title,`Релиз: ${fmt(g.releaseDate,g.releaseDateStatus)}`,`EA: ${fmt(g.earlyAccessDate,g.earlyAccessDateStatus)}`,labeledInlineSelect('Платформа:',createInlineSelect(g,'platform',PLATFORM_OPTIONS)),labeledInlineSelect('Статус:',createInlineSelect(g,'status',STATUS_OPTIONS)),actions(g)]); c.append(a);});}
function renderStats(){let known=games.filter(g=>g.releaseDate).length, dated=games.filter(g=>g.releaseDate).sort((a,b)=>a.releaseDate.localeCompare(b.releaseDate)); let p=[...new Set(games.map(g=>g.platform).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'ru')).map(v=>`<div>${v}: <b>${games.filter(g=>g.platform===v).length}</b></div>`).join(''); $('#stats').innerHTML=`<div>Всего: <b>${games.length}</b></div><div>С датой релиза: <b>${known}</b></div><div>Без даты: <b>${games.length-known}</b></div><div>Ранний доступ: <b>${games.filter(g=>g.earlyAccessDate).length}</b></div><div>Прошёл: <b>${games.filter(g=>g.status==='Прошёл').length}</b></div><div>Дропнул: <b>${games.filter(g=>g.status==='Дропнул').length}</b></div><div>Без статуса: <b>${games.filter(g=>!g.status).length}</b></div><div>Самая старая: <b>${dated[0]?.title||'—'}</b></div><div>Самая новая: <b>${dated.at(-1)?.title||'—'}</b></div>${p}`;}
function openForm(g){$('#gameForm').reset(); $('#editId').value=g?.id||''; $('#formTitle').textContent=g?'Редактировать игру':'Добавить игру'; $('#titleInput').value=g?.title||''; $('#releaseInput').value=g?.releaseDate||''; $('#eaInput').value=g?.earlyAccessDate||''; $('#releaseStatus').value=g?.releaseDateStatus||'unknown'; $('#eaStatus').value=g?.earlyAccessDateStatus||'unknown'; ensureSelectOption($('#platformInput'),g?.platform||''); $('#platformInput').value=g?.platform||''; $('#statusInput').value=g?.status||''; $('#sourceInput').value=g?.sourceUrl||''; $('#editor').showModal();}
$('#gameForm').onsubmit=e=>{e.preventDefault(); let id=$('#editId').value||String(Date.now()), title=$('#titleInput').value.trim(); if(!title)return; let dup=games.find(g=>g.id!==id&&g.title.toLowerCase()===title.toLowerCase()); if(dup&&!confirm('Игра с таким названием уже есть. Всё равно сохранить?'))return; let old=games.find(x=>x.id===id)||{}; let g=normalizeGame({...old,id,title,releaseDate:$('#releaseInput').value,releaseDateStatus:$('#releaseStatus').value,earlyAccessDate:$('#eaInput').value,earlyAccessDateStatus:$('#eaStatus').value,platform:$('#platformInput').value,status:$('#statusInput').value,sourceUrl:$('#sourceInput').value,addedAt:old.addedAt||new Date().toISOString().slice(0,10)}); games=games.some(x=>x.id===id)?games.map(x=>x.id===id?g:x):[g,...games]; save(); $('#editor').close(); render();};
function download(name, data, type){let a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([data],{type})); a.download=name; a.click(); URL.revokeObjectURL(a.href)}
function csv(rows){let esc=v=>'"'+String(v??'').replaceAll('"','""')+'"'; return ['Название,Полный релиз,Ранний доступ,Платформа,Статус'].concat(rows.map(g=>[g.title,g.releaseDate,g.earlyAccessDate,g.platform,g.status].map(esc).join(','))).join('\n')}
$$('th[data-sort]').forEach(th=>th.onclick=()=>{prefs.dir=prefs.sort===th.dataset.sort&&prefs.dir==='asc'?'desc':'asc'; prefs.sort=th.dataset.sort; render();});
['search','hasEa','unknown','decade','platformFilter'].forEach(id=>$('#'+id).oninput=e=>{let m={search:'q',hasEa:'hasEa',unknown:'unknown',decade:'decade',platformFilter:'platform'}[id]; prefs[m]=e.target.type==='checkbox'?e.target.checked:e.target.value; render();});
$('#resetFilters').onclick=()=>{Object.assign(prefs,{q:'',hasEa:false,unknown:false,decade:'',platform:'',statusTab:'all'});render()}; $('#themeBtn').onclick=()=>{prefs.theme=prefs.theme==='dark'?'light':'dark';render()}; $('#viewBtn').onclick=()=>{prefs.view=prefs.view==='table'?'cards':'table';render()}; $('#addBtn').onclick=()=>openForm(); $('#cancelBtn').onclick=()=>$('#editor').close(); $('#clearDates').onclick=()=>{$('#releaseInput').value=$('#eaInput').value=''}; $('#exportJson').onclick=()=>download('game-library.json',JSON.stringify({version:DATA_VERSION,games:games.map(exportGame)},null,2),'application/json'); $('#exportCsv').onclick=()=>download('game-library.csv',csv(filtered()),'text/csv;charset=utf-8'); $('#restoreBtn').onclick=()=>{if(confirm('Заменить данные исходным списком?')){lastMigrationSteamCount=0;games=clone(ORIGINAL_GAMES).map(normalizeGame);save();render();alert('Исходный список восстановлен.');}}; $('#randomBtn').onclick=()=>{let r=filtered(), g=r[Math.floor(Math.random()*r.length)]; if(g){$('#randomBox').classList.remove('hidden'); text($('#randomBox'),`Случайная игра: ${g.title}`)}}; $('#missingBtn').onclick=()=>{prefs.unknown=true;render();scrollTo(0,document.querySelector('.table-wrap').offsetTop)};
$('#importJson').onchange=e=>{let f=e.target.files[0]; if(!f)return; f.text().then(t=>{try{let raw=JSON.parse(t), a=Array.isArray(raw)?raw:raw.games; if(!Array.isArray(a)||!a.every(x=>x&&typeof x.title==='string'))throw Error(); let warnings=[]; a.forEach(x=>{if(x.platform&&!PLATFORMS.includes(x.platform))warnings.push(`Неизвестная платформа у «${x.title}»: ${x.platform}`); if(x.status&&!STATUSES.includes(x.status))warnings.push(`Неизвестный статус у «${x.title}»: ${x.status}`)}); if(confirm('Импорт полностью заменит текущую библиотеку. Продолжить?')){lastMigrationSteamCount=0;games=a.map(normalizeGame);save();render();alert(['Импорт завершён.',...warnings].join('\n'));}}catch{alert('JSON повреждён или формат не поддерживается. Нужен массив игр или объект {version, games}.')}})};
render();
