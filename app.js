...
function renderCards(rows){let c=$('#cards'); c.textContent=''; rows.forEach(g=>{let a=document.createElement('article'); a.className='card'; cells(a,[g.title,`Релиз: ${fmt(g.releaseDate,g.releaseDateStatus)}`,`EA: ${fmt(g.earlyAccessDate,g.earlyAccessDateStatus)}`,labeledInlineSelect('Платформа:',createInlineSelect(g,'platform',PLATFORM_OPTIONS)),labeledInlineSelect('Статус:',createInlineSelect(g,'status',STATUS_OPTIONS)),actions(g)]); c.append(a);});}
