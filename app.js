...
let games=loadData(), prefs=Object.assign({sort:'title',dir:'asc',view:'table',theme:'dark',q:'',hasEa:false,unknown:false,decade:'',platform:'',statusTab:'all'}, JSON.parse(localStorage.getItem(LS_PREF)||'{}'));
...
function createInlineSelect(game, field, options){let select=document.createElement('select'); select.className='inline-select'; select.setAttribute('aria-label',field==='platform'?'Платформа':'Статус'); options.forEach(({value,label})=>{let option=document.createElement('option'); option.value=value; option.textContent=label; select.append(option);}); select.value=game[field]||''; select.onchange=()=>{let next=field==='platform'?validPlatform(select.value):validStatus(select.value); if(game[field]===next)return; games=games.map(g=>g.id===game.id?{...g,[field]:next}:g); save(); render();}; return select}
